...
/** Multi-image gallery uploader for a restaurant profile. */
export function OutletGalleryUpload({
  value,
  onChange,
  max = 30,
}: {
  ...
  async function handleFiles(files: FileList) {
    ...
    try {
      const uploaded = await Promise.all(picked.map((f) => uploadVendorImage(f, "food-outlets")));
      ...
      <div className="grid grid-cols-3 gap-2 sm:grid-cols-4">
        ...
        {value.length < max && (
          <button
            ...
            className="flex aspect-square flex-col items-center justify-center gap-1 rounded-lg border border-dashed border-surface-border text-ink-faint transition hover:border-vibe-violet hover:text-vibe-violet"
          >
